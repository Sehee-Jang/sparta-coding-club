...
async function fetchComments() {
  try {
    ...
    // .json도 완성된 결과를 받으려면 기다려야함
    const comments = await response.json();

    // 3. 여기에 slice 메서드를 활용하여 API에서 받아온 결과 배열에서
    //     0번째부터 9번째 값까지를 복사한 새로운 배열인 slicedData를 만들어주세요.
    let slicedData = comments.slice(0, 10); // 0번째부터 10번째 전까지
    const commentsDiv = document.getElementById("comments");

    // slicedData 배열의 각 comment에 대해 반복
    slicedData.forEach((comment) => {
      let slicedBody = "";
      console.log("comment >> ", comment);

      // 4. 여기에 slice 메서드를 활용하여 comment의 body의 길이가 40 이상일 때
      // 0번째부터39번째 문자를 복사하고 맨 뒤에 "..."을 붙인 문자열을slicedBody라는 변수에 할당해주세요.
